...
<form action="http://localhost/spring_mvc/mybatis/day0530/updateProc.do" method="get" id="frm">
	<label>변경사항 입력</label>
...
<c:if test="${ not empty param.num}">
	<c:choose>
		<c:when test="${updateVO.row_cnt ne 0 }">
			<c:out value="${updateVO.num}"/>번 님의 정보가 변경되었습니다.
		</c:when>
		<c:otherwise>
			<c:out value="${updateVO.num}"/>번 님의 정보가 변경되지 않았습니다.
			<c:out value="${updateVO.errmsg}"/>
		</c:otherwise>
	</c:choose>
</c:if>
