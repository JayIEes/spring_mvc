...
<body>
<span style="font-size:20px;"><c:out value="${msg }"/></span><br/>
이름 : <strong><c:out value="${ param.name }"/></strong><br/>
나이 : <strong><c:out value="${ param.age }"/></strong><br/>
<%-- 언어 : <strong><c:out value="${ param.lang }"/></strong><br/> --%> <!-- 여러 개면 처음 값만 가지고옴 -->
언어: 
<c:if test="${empty param.lang }">선택언어 없음</c:if>
<c:forEach var="lang" items="${paramValues.lang}"> <c:out value="${lang }"/> </c:forEach>
<br/>
